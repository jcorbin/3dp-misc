...
mode = 100; // [0:Assembly, 1:Case, 2:Case Solid, 3: Case_Cutout, 4: Fan Cowl, 100:Dev]
...
module main() {
  ...
  if (mode == 0) {
    ...
    shell(floor_cutout="yes");
  } else if (mode == 2) {
    shell(floor_cutout="");
  } else if (mode == 3) {
    shell(floor_cutout="only");
  } else if (mode == 4) {
    fan_cowl(
      orient=DOWN,
      extra=[24, 24],
          ...
          cyl(d=fan_spindle.x, h=spar.y, chamfer2=chamfer)
            attach(CENTER, CENTER)
              zrot_copies(rots=half_rots(4))
                cuboid(
                  [fan_cowl_outlet_size.x + 2 * $eps + 2 * chamfer, spar.x, spar.y], chamfer=min(spar) / 4, edges=[
                    [0, 0, 1, 1], // yz -- +- -+ ++
                    [0, 0, 0, 0], // xz
                    [0, 0, 0, 0], // xy
                  ]
...
function half_rots(n) = [for (i = [0:n]) i * 180 / n];
...
function shell(
  ...
  let (
    ...
    psu = power_supply(),
    psu_size = struct_val(psu, "size"),
    psu_room = psu_gap + psu_size.z,

    // bottom half maths ; TODO top half
    inner_size = sum(transpose(shell_inner_margin)) + mainboard_bounds + [0, psu_room, 0] - [0, 0, split_at],

    // TODO Z += 3
    outer_size = inner_size + v_mul([2, 2, 1], scalar_vec3(wall)),
    mainboard_part = define_part(
      "mainboard",
      attach_geom(size=mainboard_bounds),
      T=move(mainboard_offest) * back((psu_size.z + psu_gap) / 2) * down((inner_size.z - mainboard_bounds.z - wall) / 2) * left((inner_size.x - mainboard_size.x) / 2),
      inside=true
    ),
    psu_part = define_part(
      "power_supply",
      attach_geom(
        size=[
          psu_size.y,
      ...
      T=fwd((mainboard_size.y + psu_gap) / 2) * down((inner_size.z - psu_size.x - wall) / 2) * left((inner_size.x - psu_size.y) / 2),
      inside=true
    ),
    sidewall_size = [
      inner_size.x - 2 * wall,
    ...
    sidewall_chamfer = sidewall_size.z * 9 / 20,
  ) [
      ["outer_size", outer_size],
      ["inner_size", inner_size],

      ["mainboard", mainboard],
      ["mainboard_bounds", mainboard_bounds],
      ["mainboard_part", mainboard_part],
      ...
      ["sidewall_size", sidewall_size],
      ["sidewall_chamfer", sidewall_chamfer],
  ];
...
module shell(
  ...
  mainboard = struct_val(info, "mainboard");
  mainboard_bounds = struct_val(info, "mainboard_bounds");
  mainboard_part = struct_val(info, "mainboard_part");
  psu_part = struct_val(info, "psu_part");
  inner_size = struct_val(info, "inner_size");
  outer_size = struct_val(info, "outer_size");
  sidewall_size = struct_val(info, "sidewall_size");
  sidewall_chamfer = struct_val(info, "sidewall_chamfer");

  attachable(
    anchor, spin, orient, size=outer_size, parts=[
      define_part(
        "inner",
    ...
    diff() cuboid(
            ...
            cuboid(
              ...
              up(wall / 2)
                attach([FRONT, BACK], FRONT, overlap=$eps)
                  cuboid(
                    sidewall_size + [0, 2 * $eps, $eps],
                    chamfer=sidewall_chamfer,
                    edges="Y"
                  );
            }
        ...
        mount_holes = let (
          components = struct_val(mainboard, "components"),
            ...
            for (i = [0:3]) struct_val(components, str("mount_hole_", i)),
        ];

        // mainboard mount posts
        restore_part(mainboard_part) let (
          ...
          restore_part(mainboard_part) {
            path = round_corners(
              xflip(
                offset(
                  [
                    for (h = mount_holes) struct_val(h, "offset"),
                  ], r=-4
                )
              ),
...
module shell_cap(
  ...
  shell_info = shell();
  outer_size = struct_val(shell_info, "outer_size");
  sidewall_size = struct_val(shell_info, "sidewall_size");
  sidewall_chamfer = struct_val(shell_info, "sidewall_chamfer");
  ...
  attachable(
    ...
    down(wall)
    cuboid(
      [outer_size.x, outer_size.z, wall],
      chamfer=wall/8, edges=[
        [1, 1, 1, 1], // yz -- +- -+ ++
        [1, 1, 1, 1], // xz
        [1, 1, 1, 1], // xy
      ],
...
function pcb_part_size(info, pcb_h) =
  let (
    size = struct_val(info, "size"),
    d = struct_val(info, "d"),
...
function pcb_part_shape(info, pcb_h) =
  let (
...
module pcba(info, null = false, anchor = CENTER, spin = 0, orient = UP) {
  ...
  attachable(
    anchor, spin, orient, size=bounds, anchors=[
      for (comp = struct_val(info, "components")) let (
        name = comp[0],
        cinfo = comp[1],
        attach = struct_val(cinfo, "attach"),
      ) if (is_def(attach)) let (
