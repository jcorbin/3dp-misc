...
mode = 100; // [0:Assembly, 1:Bottom, 2:Top, 3:Latch, 4:Gasket, 5:Bottom Insert, 6:Top Insert, 50:Test Gasket, 51:Test Wall, 100:Dev]

// TODO latch test fixture
...
// The chamfer radius of the boxes corners.  (NOTE[1]: the floor/top radius is slightly different than the sides to eliminate the need for supports. NOTE[2]: If you want square inside corners, the boxWallWidthMm must be >= the boxChamferRadiusMm.)
...
// These inserts are meant for customizing your own holders for custom parts inside the cause.  Likely printed in TPU. The insert will fill the entire top/bottom of the case, then you can use the CAD tool of your choice to cutout spaces for your stuff.
...
module main() {
  ...
  else if (mode == 5) { // Bottom Insert
    translate([
      boxWidthXMm + 20,
      0,
      boxBottomHeightZMm-boxWallWidthMm-insertTolerance
    ])
    resize([
      boxWidthXMm-(boxWallWidthMm*2)-(insertTolerance*2),
      boxLengthYMm-(boxWallWidthMm*2)-(insertTolerance*2),
      boxBottomHeightZMm-boxWallWidthMm-insertTolerance
    ])
      difference() {
  ...
  else if (mode == 6) { // Top Insert
    translate([
      boxWidthXMm + 20,
      boxLengthYMm + 20,
      boxTopHeightZMm-boxWallWidthMm-insertTolerance
    ])
    resize([
      boxWidthXMm-(boxWallWidthMm*2)-(insertTolerance*2),
      boxLengthYMm-(boxWallWidthMm*2)-(insertTolerance*2),
    ...
    translate([(-internalBoxWidthXMm/2)-(boxWallWidthMm*4), 0, rimHeightMm])
      difference() {
        ...
        translate([boxWallWidthMm*4, -boxLengthYMm/2, -boxBottomHeightZMm/2])
          cube([2*boxWidthXMm, 2*boxLengthYMm, 2*boxBottomHeightZMm]);
        translate([-boxWidthXMm/2, boxLengthYMm/2, -boxBottomHeightZMm/2])
          cube([2*boxWidthXMm, 2*boxLengthYMm, 2*boxBottomHeightZMm]);
      }
  ...
  else if (mode == 51) { // Test Wall
    translate([0,-50,0])
    rotate([90,0,0])
    linear_extrude(6)
      Wall2D(boxWallWidthMm, 0, boxBottomHeightZMm, true, true);
  }
...
// TODO reconsider feet someday
//
// // Should feet be generated and the feet connections be cutout from the container top and bottom.  (NOTE: this will require some glue... sorry, no time to create a snap-in connector)
// isFeetAdded = false;
